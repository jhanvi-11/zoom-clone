...
def create_instant_meeting(db: Session, meeting_in: MeetingCreateInstant):
    ...
    new_meeting = Meeting(
        ...
        invite_link=f"http://localhost:3000/j/{meeting_id.replace(' ', '')}"
    )
    db.add(new_meeting)
    db.commit()
...
def get_recent_meetings(db: Session, user_id: int):
    ...
    for p in participations:
        if p.meeting_id not in meeting_ids:
            meeting_ids.append(p.meeting_id)
            if p.meeting.status == MeetingStatus.ended or p.meeting.status == MeetingStatus.active:
